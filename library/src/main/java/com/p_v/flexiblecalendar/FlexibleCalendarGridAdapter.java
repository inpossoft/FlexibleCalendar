...
class FlexibleCalendarGridAdapter extends BaseAdapter {
    ...
    private MonthViewPagerAdapter.CalendarType type;
    private List<Date> datesWithEvents;

    private static final int SIX_WEEK_DAY_COUNT = 42;


    public FlexibleCalendarGridAdapter(Context context, int year, int month,
                                       boolean showDatesOutsideMonth, boolean decorateDatesOutsideMonth, int startDayOfTheWeek, MonthViewPagerAdapter.CalendarType type, List<Date> datesWithEvents) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (type == MonthViewPagerAdapter.CalendarType.Single) {
            ...
            if (isWithinCurrentMonth) {
                //set to REGULAR if is within current month
                cellType = BaseCellView.REGULAR;
                if (datesWithEvents != null) {
                    for (Date date : datesWithEvents) {
                        Calendar cal = new GregorianCalendar();
                        cal.setTime(date);

                        if (cal.get(Calendar.DAY_OF_MONTH) == day && cal.get(Calendar.YEAR) == year && cal.get(Calendar.MONTH) == month) {
                            cellType = BaseCellView.SELECTED;
                        }
                    }
                }
